...
def _download_sp500_list():
    ...
    with urllib.request.urlopen(SP500_LIST_URL) as f:
        print("Downloading ...", SP500_LIST_URL)
        with open(SP500_LIST_PATH, "w") as fin:
            print(f.read().decode(), file=fin)
    return
...
def fetch_prices(symbol, out_name):
    ...
    symbol_url = BASE_URL.format(
        urllib.parse.quote(symbol), urllib.parse.quote(now_datetime, "+")
    )
    print("Fetching {} ...".format(symbol))
    print(symbol_url)

    try:
        with urllib.request.urlopen(symbol_url) as f:
...
def main(continued=False):
    ...
    for idx, sym in enumerate(symbols):
        ...
        if idx % 10 == 0:
            print(
                "# Failures so far [%d/%d]: %d" % (idx + 1, len(symbols), num_failure)
            )
